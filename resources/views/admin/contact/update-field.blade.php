...
<div class="card">
    ...
    <div class="card-body">
        ...
        <form method="post" action="{{ route('admin.contact.update_field', ['field_id' => $field->id]) }}">
            ...
            @foreach ($field->langs as $field_lang)
                <h5 class="mb-3">
                    @if (count($languages) > 1)
                        {!! flag($field_lang->lang->code) !!} {{ $field_lang->lang->name }}
                    @endif
                </h5>

                <div class="form-group">
                    <label>{{ __('Label') }}</label>
                    <input class="form-control" name="label_{{ $field_lang->lang->id }}" value="{{ $field_lang->label ?? null }}" required>
                </div>

                <div class="form-group">
                    <label>{{ __('Info') }} ({{ __('optional') }})</label>
                    <textarea class="form-control" rows="2" name="info_{{ $field_lang->lang->id }}">{{ $field_lang->info ?? null }}</textarea>
                    <div class="text-muted small">{{ __('If set, info text will be visible below the form field.') }}</div>
                </div>

                <div id="hidden_div_dropdowns_{{ $field->id }}_{{ $field_lang->lang->id }}" style="display: @if ($field->type == 'select' || $field->type == 'checkbox') block @else none @endif">
                    <div class="form-group">
                        <label>{{ __('Values') }} ({{ __('each option in a new line') }})</label>
                        <textarea class="form-control" rows="4" name="dropdowns_{{ $field_lang->lang->id }}">{{ $field_lang->dropdowns ?? null }}</textarea>
                    </div>
                </div>
