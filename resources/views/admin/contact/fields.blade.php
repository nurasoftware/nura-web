                            ...
                            <td>
                                ...
                                @foreach ($field->langs as $field_lang)
                                    @if (count($languages) > 1)
                                        {!! flag($field_lang->lang->code) !!}
                                    @endif

                                    @if (!($field_lang->label ?? null))
                                        <span class="text-danger">{{ __('Label not set') }}</span>
                                    @else
                                        <b>{{ $field_lang->label }}</b>
                                    @endif

                                    @if ($field->type == 'select' || $field->type == 'checkbox')
                                        <div class="mb-1"></div>
                                        @if (!($field_lang->dropdowns ?? null))
                                            <span class="text-danger">{{ __('Values not set') }}</span>
                                        @else
                                            {{ __('Values') }}: <br>{!! nl2br($field_lang->dropdowns) !!}
                                        @endif
                                    @endif

                                    @if ($field_lang->info ?? null)
                                        <br><i>{{ $field_lang->info }}</i>
                                    @endif
