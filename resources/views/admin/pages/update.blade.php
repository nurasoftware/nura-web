                ...
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('admin.pages.show', ['id' => $page->id]) }}">
                            @if ($page->is_homepage == 0)
                                {{ $page->default_language_content->title }}
                            @else
                                {{ __('Homepage') }}
                            @endif
                        </a></li>
    ...
    <div class="card-body">
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            @if ($page->is_homepage == 1)
                <div class="fw-bold text-success mb-2"><i class="bi bi-info-circle"></i> {{ __('This page is main website homepage.') }}</div>
                                    ...
                                    @foreach ($root_pages as $root_page)
                                        @if (!$root_page->is_homepage)
                                            @if ($page->id != $root_page->id)
                                                <option @if ($page->parent_id == $root_page->id) selected @endif value="{{ $root_page->content->page_id }}">
                                                    {{ $root_page->content->title }}
                                                </option>
                                            @endif
                                        @endif
                                    @endforeach
            ...
            @if ($page->is_homepage == 1)
                ...
                @foreach ($contents as $language)
                    @if (count($contents) > 1)
                        <div class="fw-bold">{!! flag($language->code) !!} {{ $language->name }}</div>
                    @endif

                    <div class="form-group">
                        <label>{{ __('Website meta title') }} </label>
                        <input type="text" class="form-control" name="meta_title_{{ $language->id }}" value="{{ $language->page_content->meta_title ?? null }}" maxlength="200">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="meta_description_{{ $language->id }}" value="{{ $language->page_content->meta_description ?? null }}">
                        <div class="text-muted small">{{ __('Include relevant information about the website content in the description.') }}</div>
                    </div>

                    @if (count($contents) > 1 && !$loop->last)
                        <hr>
                    @endif
                @endforeach
            @else
                @foreach ($contents as $lang)
                    ...
                    @if (!($page->is_homepage == 1 || $page->is_contactpage == 1))
                        <div class="form-group">
                            ...
                            <input class="form-control" name="title_{{ $lang->id }}" type="text" value="{{ $lang->page_content->title ?? null }}" @if ($lang->is_default == 1) required @endif>
                        </div>
                    @endif
                    ...
                    <div class="row">
                        <div class="col-md-4 col-sm-12">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="meta_title_{{ $lang->id }}" value="{{ $lang->page_content->meta_title ?? null }}">
                        ...
                        <div class="col-md-4 col-sm-12">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" name="meta_description_{{ $lang->id }}" value="{{ $lang->page_content->meta_description ?? null }}">
                            </div>
                        </div>
                        ...
                        @if ($page->is_homepage == 0)
                            <div class="col-md-4 col-sm-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control customSlug" name="slug_{{ $lang->id }}" value="{{ $lang->page_content->slug ?? null }}">
