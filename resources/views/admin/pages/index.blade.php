            ...
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    @foreach ($pages as $page)
                        <tr @if ($page->is_homepage == 1) class="table-light" @endif @if ($page->active == 0) class="table-warning" @endif>
                            ...
                            <td>
                                ...
                                @if (!$page->is_homepage == 1)
                                    @foreach ($page->translations as $lang_content)
                                        @if ($lang_content->lang_status == 'disabled')
                                            @php continue @endphp
                                        @endif

                                        @if (count($languages) > 1)
                                            {!! flag($lang_content->lang_id) !!}
                                        @endif

                                        @if ($page->active == 1)
                                            <a target="_blank" href="{{ $lang_content->url }}">
                                                <span @if (($lang_content->language->is_default ?? null) == 1) class="fw-bold fs-6" @endif>
                                                    @if ($page->parent_id)
                                                        <i class="bi bi-arrow-return-right"></i>
                                                    @endif
                                                    <b>{{ $lang_content->title }}</b>
                                                </span>
                                            </a>
                                        @else
                                            <span @if (($lang_content->language->is_default ?? null) == 1) class="fw-bold fs-6" @endif>
                                ...
                                <small class='text-muted'>
                                    ...
                                    @if ($page->parent_id)
                                        ...
                                        <b>{{ __('Parent page') }}:</b> <a target="_blank" href="{{ $page->parent->content->url ?? null }}">{{ $page->parent->content->title }}</a>
                                        <div class="clearfix"></div>
                                    @endif
