...
?>
<div class="modal fade custom-modal" tabindex="-1" role="dialog" aria-hidden="true" aria-labelledby="addBlockLabel{{ $pos ?? null }}" id="addBlock{{ $pos ?? null }}">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">

            @php
                if (($module ?? null) == 'pages') {
                    $action = route('admin.pages.content.new', ['id' => $page->id]);
                } elseif (($module ?? null) == 'footer') {
                    $action = route('admin.template.footer.content.new', ['footer' => $footer, 'pos' => $pos]);
                }
            @endphp
